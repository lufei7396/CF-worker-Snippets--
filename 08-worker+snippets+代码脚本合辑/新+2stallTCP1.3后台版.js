/**
 * StallTCP1.3 修复版 + 紫色UI + 经典布局还原
 * 
 * 1. [核心] 保持 StallTCP 1.3 核心逻辑不动。
 * 2. [订阅] 
 *    - 还原了订阅后端地址输入框。
 *    - Worker 内置 /sub 路径，读取代码中的 DEFAULT_CUSTOM_IPS。
 * 3. [布局] 还原为：后端地址 -> ProxyIP -> 结果链接。
 */
...
// 2. 默认配置
const DEFAULT_SUB_DOMAIN = "sub.cmliussss.net";      // 默认外部订阅后端域名
...
export default {
  async fetch(r) { 
    ...
    // --- [Worker 内置订阅处理 /sub] ---
    // 只有当访问本 Worker 的 /sub 路径时，才会触发此逻辑
    // 这允许你将 "订阅后端地址" 设置为本 Worker 域名，从而分发 DEFAULT_CUSTOM_IPS
    if (url.pathname === '/sub') {
...
<div class="container">
  <div class="card">
    ...
    <div class="field">
      ...
      <div class="warn">提示：如果填写本 Worker 域名 (https://${host})，将使用代码内置的优选IP生成节点。</div>
    </div>
  ...
  <div class="card">
    ...
    <div class="field">
      ...
      <div class="desc" style="color: #a29bfe;">⚠️ 注意：若要修改优选IP节点，请在 Worker 源代码中修改 DEFAULT_CUSTOM_IPS 变量。该列表仅在订阅后端为本 Worker 域名时生效。</div>
    </div>
  </div>
</div>
...
function updateLink() {
  ...
  // 构造清洁链接
  const subUrl = \`\${baseUrl}/sub?uuid=\${uuid}&encryption=none&security=tls&sni=\${host}&alpn=h3&fp=random&allowInsecure=1&type=ws&host=\${host}&path=\${encodedPath}\`;
  
  document.getElementById('resultUrl').value = subUrl;
}
