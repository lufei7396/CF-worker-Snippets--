...
/**
 * StallTCP1.3 修复版 + 紫色UI + 纯净订阅源 + 自定义订阅转换
 * 
 * 1. [核心] 保持 StallTCP 1.3 核心逻辑不动。
 * 2. [订阅] Worker 内置 /sub 仅返回纯净 Base64 (彻底删除所有格式参数处理)。
 * 3. [转换] UI 集成外部转换器拼接逻辑，支持自定义后端和 ACL4SSR 配置文件。
 * 4. [兼容] Cloudflare Workers & Snippets (无环境变量)。
 */
...
// 🟣 用户配置区域 (在此处修改配置)
// =============================================================================
const UUID = "ed22fdaf-81b5-439d-ac0e-8a5f5ac28d33"; // 请修改你的可用UUID
...
export default {
  async fetch(r) { 
    ...
    // =========================================================================
    // 🟢 Worker 内置订阅处理 (/sub)
    // 只返回纯净 Base64，删除所有 target/format 参数处理
    // =========================================================================
    if (url.pathname === '/sub') {
        ...
        // 返回纯 Base64 文本
        return new Response(btoa(unescape(encodeURIComponent(nodeList.join('\n')))), {
            status: 200,
  ...
  <div class="card">
    ...
    <div class="field">
      ...
      <div class="checkbox-wrapper">
        ...
        <label for="clashMode">🔄 开启 Clash 订阅转换 (使用转换器 + ACL4SSR 规则)</label>
      </div>
    </div>
...
function updateLink() {
  ...
  if (isClash) {
     ...
     // 构造标准转换链接
     // 格式: https://转换后端/sub?target=clash&url=原始链接&config=ACL4SSR配置
