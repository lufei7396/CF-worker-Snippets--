/**
 * StallTCP1.3修复版 (Shadowrocket兼容版)
 * 
 * 修改记录：
 * 1. 移除了 fetch 阶段的预先 send([0,0])，解决 Shadowrocket 连接失败问题。
 * 2. 将 send([0,0]) 移动至 handle 内部，在收到第一个数据包并验证 UUID 后发送。
 * 
 * 原功能：
 * 修复日常的小数据请求更重要的逻辑问题。
...
export default {
  async fetch(r) { 
    ...
    // --- 修改处1：删除了 s.send(new Uint8Array([0, 0])); ---
