...
function isSpeedTestSite(hostname) {
    ...
    return speedTestDomains.some(d => hostname === d || hostname.endsWith('.' + d));
}
    ...
    async fetch(request,env) {
        try {
            ...
            if (request.headers.get('Upgrade') === 'websocket') {
                ...
                // Clash 订阅路径 /clash/UUID
                if (url.pathname.toLowerCase() === `/clash/${subPath.toLowerCase()}` || url.pathname.toLowerCase() === `/clash/${subPath.toLowerCase()}/`) {
                    const d = url.hostname, ps = [], ns = [];
                    cfip.forEach((c, idx) => {
                        let h, p = 443, n = '';
                        if (c.includes('#')) [c, n] = c.split('#');
                        if (c.startsWith('[') && c.includes(']:')) {
                            const i = c.indexOf(']:');
                            h = c.substring(1, i);
                            p = parseInt(c.substring(i + 2)) || 443;
                        } else if (c.includes(':')) {
                            [h, p] = c.split(':');
                            p = parseInt(p) || 443;
                        } else h = c;
                        const pn = n ? `${n}-SS-${idx + 1}` : `SS-${idx + 1}`;
                        ns.push(pn);
                        ps.push(`  - {name: "${pn}", type: ss, server: ${h}, port: ${p}, cipher: ${method}, password: "${password}", plugin: v2ray-plugin, plugin-opts: {mode: websocket, tls: true, skip-cert-verify: true, host: ${d}, path: "/?ed=2560", mux: false}, udp: false}`);
                    });
                    const y = `port: 7890\nsocks-port: 7891\nallow-lan: false\nmode: Rule\nlog-level: info\nexternal-controller: 127.0.0.1:9090\nproxies:\n${ps.join('\n')}\nproxy-groups:\n  - {name: "🚀 节点选择", type: select, proxies: ["♻️ 自动选择", "🔰 故障转移", DIRECT, ${ns.join(', ')}]}\n  - {name: "♻️ 自动选择", type: url-test, proxies: [${ns.join(', ')}], url: "http://www.gstatic.com/generate_204", interval: 300}\n  - {name: "🔰 故障转移", type: fallback, proxies: [${ns.join(', ')}], url: "http://www.gstatic.com/generate_204", interval: 300}\nrules:\n  - DOMAIN-SUFFIX,local,DIRECT\n  - IP-CIDR,127.0.0.0/8,DIRECT\n  - IP-CIDR,172.16.0.0/12,DIRECT\n  - IP-CIDR,192.168.0.0/16,DIRECT\n  - IP-CIDR,10.0.0.0/8,DIRECT\n  - GEOIP,CN,DIRECT\n  - MATCH,🚀 节点选择`;
